...
export const travelerDetailsSchema = z
  .object({
    // Required fields
    name: z
      .string()
    ...
    residencyStatus: z.enum(
      ['Citizen', 'Permanent Resident', 'Temporary Resident', 'Student Visa', 'Work Visa'],
      {
...
export const preferencesSchema = z.object({
  travelStyle: z.enum(['Relaxed', 'Balanced', 'Packed', 'Budget-Focused'], {
  ...
  dietaryRestrictions: z
    .array(z.string())
    .default([])
    .refine((restrictions) => restrictions.length <= 10, {
 ...
 */
export function getFieldErrors(errors: z.ZodError | undefined, fieldPath: string): string[] {
  if (!errors) return [];
 ...
 */
export function hasFieldError(errors: z.ZodError | undefined, fieldPath: string): boolean {

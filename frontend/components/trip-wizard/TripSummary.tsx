...
export default function TripSummary({ formData, onEdit }: TripSummaryProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="text-center py-8 bg-gradient-to-br from-blue-50 to-amber-50/30 dark:from-slate-800 dark:to-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
        <div className="text-5xl mb-4">🎉</div>
        <h2 className="text-3xl font-bold text-slate-900 dark:text-slate-50 mb-2">Almost There!</h2>
      ...
      <div className="space-y-4">
        {/* Traveler Details */}
        <SummaryCard title="Traveler Details" icon="👤" onEdit={() => onEdit(1)}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <SummaryItem
              label="Party Size"
              value={`${travelerDetails.partySize} ${travelerDetails.partySize === 1 ? 'person' : 'people'}`}
            />
        ...
        {/* Destinations */}
        <SummaryCard title="Destinations" icon="📍" onEdit={() => onEdit(2)}>
        ...
        {/* Trip Details */}
        <SummaryCard title="Trip Details" icon="🗓️" onEdit={() => onEdit(3)}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <SummaryItem
              label="Departure"
        ...
        {/* Preferences */}
        <SummaryCard title="Preferences" icon="⭐" onEdit={() => onEdit(4)}>
          <div className="space-y-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Once you submit, our AI agents will analyze visa requirements, destination
              intelligence, weather forecasts, and more to create your comprehensive travel report.
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={onEdit}
          className="px-4 py-2 text-sm font-medium text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/30 rounded-lg transition-colors flex items-center gap-2"
        >
          <svg
            className="w-4 h-4"
            fill="none"
...
function SummaryItem({ label, value }: SummaryItemProps) {
  return (
    <div>
      <div className="text-xs font-medium text-slate-500 dark:text-slate-500 mb-1">{label}</div>
